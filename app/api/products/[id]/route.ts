...
import { detectLowStockTransition } from "@/lib/services/inventory";
...
export async function PUT(
  ...
  try {
    ...
    // Capture previous stock so we can detect low-stock transitions.
    ...
    if (body.totalStock !== undefined) {
      const { data: existingStock } = await supabase
        ...
        .eq("id", id)
        .single();
      previousStock = existingStock ? Number(existingStock.stock ?? 0) : null;
    }
    ...
    const productData = { ...(fullProduct ?? updated), combo_items: comboItems ?? [] };

    // Surface low-stock transition so the UI can toast and the header bell refreshes.
    let lowStockAlert = null;
    if (previousStock !== null && body.totalStock !== undefined) {
      const threshold = getLowStockThreshold();
      lowStockAlert = detectLowStockTransition(
        previousStock,
        Number(body.totalStock),
        {
          id,
          name: productData.name ?? body.title,
    ...
    return NextResponse.json({
      ...
      stockLevel:
        body.totalStock !== undefined
          ? classifyStock(Number(body.totalStock), getLowStockThreshold())
          : undefined,
    });
  } catch (error: any) {
